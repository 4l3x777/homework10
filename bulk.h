...
{
public:
    ICommandProcessor(std::shared_ptr<ICommandProcessor> next_command_processor = nullptr) : _next_command_processor(next_command_processor) {}
...
protected:
    std::shared_ptr<ICommandProcessor> _next_command_processor;
};

class ConsoleInput : public ICommandProcessor
{
public:
    ConsoleInput(std::shared_ptr<ICommandProcessor> next_command_processor = nullptr) : ICommandProcessor(next_command_processor), _block_depth(0) {}
...
public:
    ...
    ConsoleOutput(std::shared_ptr<ICommandProcessor> next_command_processor = nullptr) : ICommandProcessor(next_command_processor) {}
...
public:
    ...
    FileOutput(std::shared_ptr<ICommandProcessor> next_command_processor = nullptr) : ICommandProcessor(next_command_processor) {}
...
{
public:
    CommandProcessor(size_t bulk_max_depth, std::shared_ptr<ICommandProcessor> next_command_processor) : ICommandProcessor(next_command_processor), _bulk_max_depth(bulk_max_depth), _used_block_command(false) {}
...
class ThreadManager
{   
    std::shared_ptr<FileOutput> file_stream{nullptr};
    std::shared_ptr<ConsoleOutput> console_stream{nullptr};
    size_t context{0};
    std::atomic<uint8_t> num_terminate_threads {0};
    bool keep_running {true};

    std::unique_ptr<std::thread> log;
    std::unique_ptr<std::thread> file1;
    std::unique_ptr<std::thread> file2;
...
public:
    ThreadManager(std::shared_ptr<FileOutput> fo, std::shared_ptr<ConsoleOutput> co, size_t _context) : file_stream(fo), console_stream(co), context(_context)
    {
        log = std::make_unique<std::thread>([&] { console_write(); });
        log->detach();
        file1 = std::make_unique<std::thread>([&] { file_write(1); });
        file1->detach();
        file2 = std::make_unique<std::thread>([&] { file_write(2); });
        file2->detach();
    }
    ...
    ~ThreadManager()
    {
        // safe exit
        keep_running = false;
        while(num_terminate_threads != 3) std::this_thread::sleep_for(std::chrono::duration<double, std::milli>(1000));    
    }
};
