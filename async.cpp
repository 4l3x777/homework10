...
class BulkHandler 
{
    std::shared_ptr<bulk::FileOutput> file_output;
    std::shared_ptr<bulk::ConsoleOutput> console_output;
    std::shared_ptr<bulk::CommandProcessor> command_processor;
    std::shared_ptr<bulk::ThreadManager> thread_manager;
    std::shared_ptr<bulk::ConsoleInput> console_input;

public:
    BulkHandler(std::size_t bulk_max_depth, std::size_t context = 0) 
    {
        file_output = std::make_shared<bulk::FileOutput>();
        console_output = std::make_shared<bulk::ConsoleOutput>(file_output);
        command_processor = std::make_shared<bulk::CommandProcessor>(bulk_max_depth, console_output);
        console_input = std::make_shared<bulk::ConsoleInput>(command_processor);
        thread_manager = std::make_shared<bulk::ThreadManager>(file_output, console_output, context);    
    }
